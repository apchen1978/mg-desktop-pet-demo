...
// Improvements over v1: real running frames (with left/right direction),
// one-shot "cute" (no infinite jump-wave loop), drag throttle to avoid
// accidental taps, and clearer state labels.
(function () {
  "use strict";

  var ASSETS = {
    idle: { frames: ["idle/00", "idle/01", "idle/02", "idle/03", "idle/04", "idle/05"], delay: 220, loop: true },
    run: { frames: ["running/00", "running/01", "running/02", "running/03", "running/04", "running/05"], delay: 85, loop: true },
  ...
  function play(name, label, opts) {
    ...
    current = name;
    pet.classList.toggle("running", name === "run" || name === "run-left" || name === "run-right");
  ...
  function startRunning(direction) {
    if (dragging) {
      var name = direction === "left" ? "run-left" : direction === "right" ? "run-right" : "run";
